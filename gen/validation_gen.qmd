---
title: "Validation Dataset Generation Code"
author: "AJ Thurston"
date: "2024-07-11"
format: html
editor: source
...
freq(df$perf_final)
```
# Data Recode - Year Hired

```{r year_hired}
df$yearh <- runif(dim(df)[1], min = 0, max = 1) %>%
  cut(30, labels = F)+1994
```
...
exp_jksonly <- df %>%
  ...
  dplyr::summarize(m = mean(perf_final)) %>%
...
irs <- tibble(
  ...
  Value = c(
    air(data = df, maj_group =  "male_f", pass_test =  "pred_jksonly_pass"),
    air(data = df, maj_group =  "male_f", pass_test =  "pred_battery_pass"),
    air(data = df, maj_group =  "white_f", pass_test =  "pred_jksonly_pass"),
    air(data = df, maj_group =  "white_f", pass_test =  "pred_battery_pass")
  )
)
...
filename <- paste("/Users/ajthurston/Documents/GitHub/validation/data/df_",format(Sys.time(), "%Y-%m-%d_%H%M"),".csv", sep = "")
write.csv(df_final, filename, row.names = FALSE) 

filename <- "/Users/ajthurston/Documents/GitHub/validation/data/validation_dataframe.csv"
write.csv(df_final, filename, row.names = FALSE) 
```
